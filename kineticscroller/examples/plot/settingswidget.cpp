...
public:
    MetricItemUpdater(MetricItem *item)
        ...
        , m_valueLabel(0)
    {
        m_easingCurveType = QVariant::fromValue<QEasingCurve>(QEasingCurve()).userType();
        m_frameRateType = QVariant::fromValue(QtScrollerProperties::Standard).userType();
        m_overshootPolicyType = QVariant::fromValue(QtScrollerProperties::OvershootWhenScrollable).userType();

        if (m_item->min.userType() == m_easingCurveType) {
            ...
            m_combo = new QComboBox();
            m_combo->addItem("Standard", QtScrollerProperties::Standard);
            m_combo->addItem("60 FPS",   QtScrollerProperties::Fps60);
            m_combo->addItem("30 FPS",   QtScrollerProperties::Fps30);
            m_combo->addItem("20 FPS",   QtScrollerProperties::Fps20);
        } else if (m_item->min.userType() == m_overshootPolicyType) {
            m_combo = new QComboBox();
            m_combo->addItem("When Scrollable", QtScrollerProperties::OvershootWhenScrollable);
            m_combo->addItem("Always On",       QtScrollerProperties::OvershootAlwaysOn);
            m_combo->addItem("Always Off",      QtScrollerProperties::OvershootAlwaysOff);
        } else {
...
private slots:
    void valueChanged(const QVariant &v)
    {
        ...
        if (m_slider) {
            ...
            if (m_item->min.userType() == m_easingCurveType) {
                m_combo->setCurrentIndex(m_combo->findData(v.value<QEasingCurve>().type()));
            } else if (m_item->min.userType() == m_overshootPolicyType) {
                m_combo->setCurrentIndex(m_combo->findData(v.value<QtScrollerProperties::OvershootPolicy>()));
            } else if (m_item->min.userType() == m_frameRateType) {
                m_combo->setCurrentIndex(m_combo->findData(v.value<QtScrollerProperties::FrameRates>()));
            }
        }
    }
    ...
    void controlChanged(int value)
    {
        ...
        if (m_slider && !combo) {
            ...
            if (m_item->min.userType() == m_easingCurveType) {
                m_value = QVariant::fromValue(QEasingCurve(static_cast<QEasingCurve::Type>(m_combo->itemData(value).toInt())));
            } else if (m_item->min.userType() == m_overshootPolicyType) {
                m_value = QVariant::fromValue(static_cast<QtScrollerProperties::OvershootPolicy>(m_combo->itemData(value).toInt()));
            } else if (m_item->min.userType() == m_frameRateType) {
                m_value = QVariant::fromValue(static_cast<QtScrollerProperties::FrameRates>(m_combo->itemData(value).toInt()));
            }
        }
        if (m_valueLabel)
...
MetricItem items[] = {
    ...
    { METRIC(ScrollingCurve),                 1,    "",         QVariant::fromValue(QEasingCurve()), 0, 0 },
    ...
    { METRIC(HorizontalOvershootPolicy),      1,    "",         QVariant::fromValue(QtScrollerProperties::OvershootWhenScrollable), 0, 0 },
    { METRIC(VerticalOvershootPolicy),        1,    "",         QVariant::fromValue(QtScrollerProperties::OvershootWhenScrollable), 0, 0 },
    { METRIC(FrameRate),                      1,    "",         QVariant::fromValue(QtScrollerProperties::Standard), 0, 0 },
};
