...
MapGraphicsWidget::MapGraphicsWidget(QGeoMappingManager *manager) :
        QGraphicsGeoMap(manager),
        panActive(false),
        kineticTimer(new QTimer)
{
    for (int i = 0; i < 5; ++i)
      mouseHistory.append(MouseHistoryEntry());
...
void MapGraphicsWidget::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
    setFocus();
...
void MapGraphicsWidget::mouseReleaseEvent(QGraphicsSceneMouseEvent* event)
{
    if (event->button() == Qt::LeftButton) {
        if (panActive) {
            panActive = false;
	    ...
	    if (entries_considered == 0) {
	      emit clicked(event->pos());
            ...
            if (entries_considered > 0)
	      kineticPanSpeed /= entries_considered;
...
void MapGraphicsWidget::mouseMoveEvent(QGraphicsSceneMouseEvent* event)
{
    if (event->modifiers() & Qt::ControlModifier) {
    } else if (panActive) {
...
void MapGraphicsWidget::kineticTimerEvent()
{
    QTime currentTime = QTime::currentTime();
    int deltaTime = lastMoveTime.msecsTo(currentTime);
    lastMoveTime = currentTime;
...
void MapGraphicsWidget::panFloatWrapper(const QPointF& delta)
{
...
void MapGraphicsWidget::mouseDoubleClickEvent(QGraphicsSceneMouseEvent *event)
{
    setFocus();
...
void MapGraphicsWidget::keyPressEvent(QKeyEvent *event)
{
    switch (event->key()) {
        case Qt::Key_Minus:
#ifdef Q_OS_SYMBIAN
...
void MapGraphicsWidget::keyReleaseEvent(QKeyEvent* event)
{
    event->accept();
...
// Evaluates the panDir field and sets kineticPanSpeed accordingly. Used in MapGraphicsWidget::keyPressEvent and MapGraphicsWidget::keyReleaseEvent
void MapGraphicsWidget::applyPan(const Qt::KeyboardModifiers& modifiers)
{
...
void MapGraphicsWidget::wheelEvent(QGraphicsSceneWheelEvent* event)
{
